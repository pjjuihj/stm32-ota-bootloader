#!/usr/bin/env python3
"""
OTA Bootloader Tool - VOFA+ 风格（标签页切换版）
"""
...
class OTAApp:
    ...
    def create_ui(self):
        ...
        self.baud_menu = ctk.CTkOptionMenu(cfg, values=["9600", "57600", "115200", "230400", "460800", "921600"],
        ...
        ctk.CTkLabel(cfg, text="流控", text_color="#666", font=ctk.CTkFont(size=10)).pack(anchor="w")
        dtr_frame = ctk.CTkFrame(cfg, fg_color="transparent")
        dtr_frame.pack(fill="x", pady=(0, 5))
        for sig in ["DTR", "RTS"]:
            ctk.CTkButton(dtr_frame, text=sig, width=50, height=24,
                          fg_color="#E0E0E0", hover_color="#D0D0D0",
                          text_color="#333", font=ctk.CTkFont(size=9)).pack(side="left", padx=2)
    ...
    def create_device_tab(self):
        ...
        info_items = [
            ("设备类型", "STM32F407VETx"),
            ("Bootloader 版本", "v1.0.0"),
            ("构建日期", "2026-07-20"),
            ("活动分区", "0x0800C000 (Partition A)"),
            ("目标分区", "0x08040000 (Partition B)"),
            ("Flash 大小", "512KB"),
            ("OTA 状态", "IDLE"),
        ]
        ...
        for label, value in info_items:
            ...
            ctk.CTkLabel(row, text=label, text_color="#666",
                         font=ctk.CTkFont(size=11), width=120, anchor="w").pack(side="left")
            ctk.CTkLabel(row, text=value, text_color="#333",
                         font=ctk.CTkFont(size=11, weight="bold")).pack(side="left")

        ctk.CTkButton(info_frame, text="刷新信息", width=100, height=32,
                      fg_color="#2196F3", hover_color="#1976D2",
                      command=self.refresh_device_info).pack(pady=15)
    ...
    def create_settings_tab(self):
        ...
        ctk.CTkLabel(auto_frame, text="启动时自动连接", text_color="#333").pack(side="left")
        self.auto_connect = ctk.CTkSwitch(auto_frame, text="", onvalue=True, offvalue=False)
        self.auto_connect.pack(side="right")
        self.auto_connect.select()

        # 自动刷新
        auto_refresh = ctk.CTkFrame(settings_frame, fg_color="transparent")
        auto_refresh.pack(fill="x", padx=15, pady=5)
        ctk.CTkLabel(auto_refresh, text="连接后自动获取设备信息", text_color="#333").pack(side="left")
        self.auto_refresh_sw = ctk.CTkSwitch(auto_refresh, text="", onvalue=True, offvalue=False)
        self.auto_refresh_sw.pack(side="right")
        self.auto_refresh_sw.select()

        # 保存日志
        save_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        save_frame.pack(fill="x", padx=15, pady=5)
        ctk.CTkLabel(save_frame, text="升级后自动保存日志", text_color="#333").pack(side="left")
        self.auto_save = ctk.CTkSwitch(save_frame, text="", onvalue=True, offvalue=False)
        self.auto_save.pack(side="right")

        # 关于
        about_frame = ctk.CTkFrame(settings_frame, fg_color="transparent")
        about_frame.pack(fill="x", padx=15, pady=15)
        ctk.CTkLabel(about_frame, text="OTA Bootloader Tool v2.0",
                     font=ctk.CTkFont(size=12, weight="bold"), text_color="#333").pack(anchor="w")
        ctk.CTkLabel(about_frame, text="STM32 OTA 升级工具",
    ...
    def switch_tab(self, tab_id):
        ...
        for tid, btn in self.tab_buttons.items():
            if tid == tab_id:
                ...
                btn.configure(fg_color="#E8E8E8", text_color="#333")
    ...
    def start_upgrade(self):
        ...
        def do_upgrade():
            try:
                ...
                self.root.after(0, lambda: self.log("启动简单工具..."))

                # 运行简单工具
                import subprocess
                cmd = [sys.executable, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ota_tool_simple.py'),
                       port, str(baud), fw_path]
